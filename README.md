...
## Qué se hizo en el TFM

El experimento final utilizó **404 imágenes/observaciones independientes**: **117 Con incidencia** y **287 Sin incidencia**. Esta composición fue la del dataset experimental del TFM y **no debe interpretarse como prevalencia operacional natural**.
...
3. **ResNet50 con transfer learning y fine-tuning**, seleccionado como modelo final del TFM.
...
Los siguientes resultados corresponden al **test histórico del dataset restringido del TFM**. Para HOG+SVM y CNN P4, precision, recall, F1 y FPR se derivan determinísticamente de las matrices históricas indicadas.
...
Estos valores son **referencias históricas del TFM**, no objetivos garantizados para un nuevo dataset. En el criterio predictivo conjunto del trabajo, ResNet50 cumplió individualmente `ROC-AUC >= 0.85` y `FPR <= 0.20`, pero **no cumplió Recall >= 0.80** (`0.7647`), por lo que el criterio conjunto **no se alcanzó**.
...
- las 404 imágenes originales del TFM;
...
Solo `alcance=completo` permite afirmar que se reconstruyó la **comparación de los tres enfoques**. Ejecutar una rama individual no demuestra el ranking comparativo del TFM ni superioridad sobre otros modelos.
...
10. Reporte las métricas obtenidas como resultados de **su propio dataset y ejecución**. No presente `tau*` ni las métricas históricas del TFM como valores transferibles.
...
- `image_id` debe ser un identificador propio de su réplica, no un ID real del TFM;
...
- **Tamaño muestral:** 404 observaciones constituyen una base limitada para estimar estabilidad y generalización. El TFM no aisló causalmente el efecto del tamaño muestral sobre el desempeño de HOG+SVM o CNN.
...
Estas líneas son propuestas de continuación; no deben confundirse con análisis ya realizados en el TFM.
